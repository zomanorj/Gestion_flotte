...
/**
 * Affiche une métrique clé avec icône et couleur.
 * @param {string} titre     - Libellé de la statistique
 * @param {string|number} valeur - Valeur à mettre en avant
 * @param {string} icone     - Emoji ou caractère icône
 * @param {string} couleur   - Classe Tailwind de couleur (ex: 'green', 'blue', 'red', 'orange')
...
export default function StatCard({ titre, valeur, icone, couleur = 'blue', sousTitre }) {
  ...
  return (
    <div className={`rounded-xl border ${p.bg} ${p.border} p-5 flex items-center gap-4`}>
      <div className={`w-14 h-14 rounded-xl ${p.icon} flex items-center justify-center text-2xl flex-shrink-0`}>
        {icone}
      </div>
      <div>
